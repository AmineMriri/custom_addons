...
class Appointment(models.Model):
    ...
    date_to = fields.Datetime(string='Date To', default=fields.Datetime.now() + timedelta(minutes=15), states=READONLY_STATES, copy=False, tracking=True)
    ...
    def default_get(self, fields):
        ...
        if 'date_to' in fields:
            diff = res['date_to'] - res['date']
            planned_duration = (diff.days * 24) + (diff.seconds/3600)
            if res.get('planned_duration') != planned_duration:
                res['planned_duration'] = planned_duration
